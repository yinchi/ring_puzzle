...
const INDENT_R      = Math.round(FLIP_R * 0.20);
...
const MAX_BEAD_SPEED_PX_MS = 0.20;                         // px per ms
...
export async function animateShiftRun(
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const easing  = steps === 1 ? "ease-in-out"
                  : isFirst     ? "ease-in"
                  : isLast      ? "ease-out"
                  :               "linear";
...
export async function animateFlip(ring: Ring): Promise<void> {
  ...
  indentEl.setAttribute("cy", String(
    flipParity === 1 ? TOP_Y + INDENT_OFFSET : TOP_Y - INDENT_OFFSET
  ));
